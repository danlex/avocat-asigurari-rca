...
`avocat-asigurari-rca.ro` with a modern template inspired by `varzaru.ro`. Deployed on GitHub Pages
at the custom domain **avocat-asigurari-rca.ro**.
...
│   ├── hero.html  rca.html  servicii.html
│   ├── despre.html  intrebari.html  testimoniale.html  contact.html
├── assets/css/style.scss   # Theme (compiled to style.css by Jekyll)
├── content/                # Source content parsed from the original site (Markdown, excluded from build)
├── _config.yml             # Site + contact settings
├── CNAME                   # Custom domain: avocat-asigurari-rca.ro
...
are centralized in `_config.yml` under `contact:` and pulled in via `{{ site.contact.* }}`.
...
3. The `CNAME` file already sets the custom domain to `avocat-asigurari-rca.ro`. In **Settings → Pages**
   confirm the custom domain shows `avocat-asigurari-rca.ro` and enable **Enforce HTTPS** once the
...
- **Apex `avocat-asigurari-rca.ro`** → four `A` records:
  `185.199.108.153`, `185.199.109.153`, `185.199.110.153`, `185.199.111.153`
  (optionally also the `AAAA` records: `2606:50c0:8000::153` … `8003::153`)
- **`www`** → `CNAME` to `<your-github-username>.github.io.`

DNS changes can take up to 24h to propagate; HTTPS certificate is issued automatically afterwards.
